  ...
  return (
    <div className="min-w-40">
      ...
      <div className="flex flex-col gap-1 text-body-sm text-slate-600">
        ...
        <span>{station.availableDocks} place{station.availableDocks !== 1 ? 's' : ''} libre{station.availableDocks !== 1 ? 's' : ''}</span>
        <span className="text-slate-400 text-caption">{station.totalDocks} emplacements au total</span>
      </div>
    </div>
  )
}

...
**Pages légales dédiées.** Trois pages publiques (accessibles sans authentification) complètent l'écran Paramètres : `/mentions-legales`, `/cgu` et `/confidentialite`. Elles sont reliées entre elles et accessibles depuis le bandeau RGPD de l'écran de connexion ainsi que depuis Paramètres. La politique de confidentialité y détaille les données collectées, les finalités, la base légale, les sous-traitants, la durée de conservation et les droits RGPD décrits dans cette section.
